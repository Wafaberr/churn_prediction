    ...
    async function uploadAndDisplayFile(event) {
        ...
        const button = document.querySelector('button[onclick="openFileDialog()"]');
    ...
    function displayDataset(responseData, filename) {
        ...
        previewSection.innerHTML = `
            <div class="flex justify-between items-center mb-4 bg-white p-4 rounded-t-xl border-b">
                ...
                <button onclick="closePreview()" class="text-gray-500 hover:text-gray-700 p-1 rounded-full hover:bg-gray-100">
    ...
    async function performDatasetDeletion() {
        ...
        try {
            const response = await fetch(`/delete_dataset?filename=${encodeURIComponent(currentDatasetToDelete)}`, {
                method: 'DELETE'
            });
