...
// Fonction pour charger et afficher les cas existants
async function loadUseCases() {
    const filename = "{{ filename }}";
    const container = document.getElementById("useCasesList");
    
    try {
        const response = await fetch(`/get_use_cases?filename=${filename}`);
        const useCases = await response.json();
        
        if (response.ok && useCases.length > 0) {
                ...
                <div class="border rounded-lg p-4 hover:shadow-md transition relative group">
                    <div class="flex justify-between items-start">
                        <div onclick="navigateToExperiment('${useCase.id}')" class="cursor-pointer flex-grow">
                            <h4 class="font-semibold">${useCase.name}</h4>
                            <p class="text-sm text-gray-600 mt-1">${useCase.description || 'Pas de description'}</p>
                            <div class="mt-2 flex gap-2">
                                <span class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">${useCase.task_type}</span>
                        ...
                        <div class="relative">
                            ...
                            <div id="dropdown-${useCase.id}" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10 hidden">
                                ...
                                <a href="#" class="block px-4 py-2 text-sm text-red-600 hover:bg-gray-100" onclick="event.stopPropagation(); confirmDelete('${useCase.id}')">
            ...
            container.innerHTML = '<p class="text-gray-500">Aucun cas d\'utilisation créé pour ce dataset</p>';
        }
    } catch (error) {
...
function confirmDelete(useCaseId) {
    const modal = document.createElement('div');
    modal.className = 'fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50';
    modal.innerHTML = `
        <div class="bg-white p-6 rounded-lg w-full max-w-md">
            <h3 class="text-lg font-semibold mb-4">Supprimer le cas d'utilisation</h3>
            <p class="mb-6">Êtes-vous sûr de vouloir supprimer ce cas d'utilisation ?</p>
            <div class="flex justify-end space-x-3">
                <button onclick="this.parentElement.parentElement.parentElement.remove()" class="px-4 py-2 rounded border border-gray-300 hover:bg-gray-50">
                    Annuler
                </button>
                <button onclick="deleteUseCase('${useCaseId}')" class="px-4 py-2 rounded bg-red-600 text-white hover:bg-red-700">
                    Supprimer
                </button>
            </div>
        </div>
...
async function deleteUseCase(useCaseId) {
    try {
        const response = await fetch(`/delete_usecase?id=${useCaseId}`, {
            method: 'DELETE'
        });
        
        if (response.ok) {
            loadUseCases(); // Recharger la liste
...
function openEditModal(useCaseId) {
    // Ici, vous devriez charger les données du cas d'utilisation
    // et remplir un formulaire de modification similaire au formulaire de création
    console.log('Ouvrir modal de modification pour', useCaseId);
    // Implémentez cette fonction selon vos besoins
}

// Fonction de navigation vers experiment
function navigateToExperiment(useCaseId) {
    window.location.href = `/newexperiment?use_case_id=${useCaseId}`;
}

// Modifiez la fonction submitUseCase pour recharger la liste après création
 ...
 function submitUseCase() {
        ...
        .then(response => {
        if (response.ok) {
            loadUseCases(); // Recharge la liste
            closeModal(); // Ferme le modal
            // Retirez la redirection directe vers experiment
        }
    })
    }
